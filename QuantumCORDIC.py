...
from FullPrototypeClassical import mult as classicalMult
...
def multGate(
    ...
    numIter = 2*int(2*np.ceil(np.log(np.sqrt(5)*n/m)/np.log((1+5**.5))/2))
...
def debugCORDIC(t: int, n_bits: int):
    ...
    inState = intListToBits([t,0,0,0,0], 4*[n_bits]+[n_bits])
...
def findBadMult(n_bits, rshift):
    ...
    for i in range(1<<n_bits):
        ...
        state = Statevector.from_label(intListToBits([i,0], n_bits))
        state = state.evolve(mult)
        qOut.append(twosCompToInt(
            stateToIntList(state, n_bits)[0], n_bits))
        cOut.append(twosCompToInt(
            classicalMult(i, 0, n_bits, rshift)[0],n_bits))


    plt.scatter([twosCompToInt(i,n_bits) for i in range(1<<n_bits)], 
                cOut, label="classical")
    plt.scatter([twosCompToInt(i,n_bits) for i in range(1<<n_bits)], 
                qOut, label="quantum")
    plt.legend()
    plt.show()
...
def main():
    ...
    cordic = quantumCORDIC(tReg, xReg, yReg, multReg, dReg, False)
